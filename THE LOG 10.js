  ...
  return `
    ...
    <div class="menu-week-panel">
      ...
      <div class="menu-week-stats">
        <div class="menu-week-stat"><div class="stat-number">${week.studyHours}h</div><div class="stat-label">Study Hours</div></div>
      ...
      <div class="menu-summary-row">
        <div class="card">
          <div class="menu-summary-tag">Best-performing subject</div>
          <div class="menu-summary-value">${strongest ? esc(strongest) : "Add an exam or lesson result"}</div>
        </div>
        <div class="card">
          <div class="menu-summary-tag">Subject needing most work</div>
          <div class="menu-summary-value">${weakest ? esc(weakest) : "Add an exam or lesson result"}</div>
        </div>
        <div class="card">
          <div class="menu-summary-tag">Recommended focus</div>
          <div class="menu-summary-value">${focus ? esc(focus) : "Add an exam or lesson result"}</div>
        </div>
      </div>
    </div>
    ...
    <section class="menu-tools">
      ...
      <p class="muted">Save your log as a file or restore a previous backup.</p>
      <div class="button-row">
        <button class="btn btn-ghost" data-action="export-data">Download Backup</button>
        <button class="btn btn-ghost" data-action="import-data">Restore Backup</button>
...
export function renderSubjectDetail(subject) {
  ...
  const best = subjectBestExam(subject);
  const lowest = subjectLowestExam(subject);
  ...
  const examRows = scores.slice().reverse().map((e) => `
    <tr>
      ...
      <td>
        <button class="btn-icon btn-sm" data-action="open-edit-exam" data-subject="${subject.id}" data-exam="${e.id}" title="Edit exam">✎</button>
        <button class="btn-icon btn-sm" data-action="delete-exam" data-subject="${subject.id}" data-exam="${e.id}" title="Delete">✕</button>
      </td>
    </tr>`).join("");
  ...
  return `
    <div class="section-head">
      ...
      <div style="display:flex;gap:8px">
        ...
        <button class="btn btn-ghost" data-action="open-add-study-session" data-subject="${subject.id}">+ Log Study Time</button>
        <button class="btn btn-primary" data-action="open-add-exam" data-subject="${subject.id}">+ Add Exam</button>
        <button class="btn-icon" data-action="open-edit-subject" data-subject="${subject.id}" title="Edit subject">✎</button>
    ...
    <div class="grid grid-4" style="margin-bottom:var(--sp-4)">
      ...
      <div class="card"><div class="stat-number">${studyHours}h</div><div class="stat-label">Study Time</div></div>
    </div>
    ...
    <div class="grid grid-3" style="margin-bottom:var(--sp-4)">
      ...
      <div class="card"><div class="stat-number" style="font-size:1.8rem">${best ?? "—"}${best !== null ? "%" : ""}</div><div class="stat-label">Best Score</div></div>
      <div class="card"><div class="stat-number" style="font-size:1.8rem">${lowest ?? "—"}${lowest !== null ? "%" : ""}</div><div class="stat-label">Lowest Score</div></div>
    </div>
    ...
    <div class="card" style="margin-top:var(--sp-4)">
      <h3 style="margin-bottom:10px">Study Sessions</h3>
      ${(subject.studySessions || []).length ? (subject.studySessions.slice().reverse().map((session) => `
        <div class="row-item">
          <div style="flex:1"><strong>${session.hours}h</strong><span class="muted" style="margin-left:8px">${fmtDate(session.date)}</span>${session.notes ? `<div class="muted" style="margin-top:4px">${esc(session.notes)}</div>` : ""}</div>
          <button class="btn-icon btn-sm" data-action="delete-study-session" data-subject="${subject.id}" data-session="${session.id}" title="Delete study session">✕</button>
        </div>`).join("")) : `<p class="muted">No study time logged yet. Add a session to track real hours.</p>`}
    </div>
  `;
}
    ...
    <div class="card" style="margin-bottom:var(--sp-2)">
      ...
      ${w.exercises.map((ex) => `
        <div class="workout-exercise-row" style="padding:6px 0;border-top:1px solid var(--white-12)">
          ...
          ${ex.notes ? `<div class="muted" style="grid-column:1/-1;font-size:0.82rem">${esc(ex.notes)}</div>` : ""}
        </div>`).join("")}
    </div>`).join("");
...
export function renderPlanner(weekOffset = 0) {
  ...
  const slotHtml = (i) => {
    ...
    return `
      <div class="priority-slot filled">
        ...
        ${t.notes ? `<div class="muted" style="font-size:0.8rem;margin-top:4px">${esc(t.notes)}</div>` : ""}
        <button class="btn btn-ghost btn-sm" data-action="open-edit-task" data-task="${t.id}" style="margin-top:8px">Edit</button>
      </div>`;
  };
  ...
  const otherRows = others.map((t) => `
    <div class="row-item ${t.completed ? "done" : ""}">
      ...
      <div style="flex:1">
        ...
        ${t.notes ? `<div class="muted" style="font-size:0.78rem;margin-top:4px">${esc(t.notes)}</div>` : ""}
      </div>
      <button class="btn-icon btn-sm" data-action="open-edit-task" data-task="${t.id}" title="Edit task">✎</button>
  ...
  const weekHtml = days.map((d, i) => {
    ...
    return `
      <div class="week-day ${isToday ? "today" : ""}">
        ...
        ${dayTasks.map((t) => `<div class="week-task ${t.completed ? "done" : ""}" title="${esc(t.notes || t.type)}">${esc(t.title)}</div>`).join("")}
  ...
  return `
    <div class="section-head">
      <h1>PLANNER</h1>
      <div class="button-row">
        <button class="btn btn-ghost" data-action="enable-reminders">Enable Reminders</button>
...
export function renderInsights(period = "week") {
  ...
  return `
    ...
    ${activityVals.some((v) => v > 0) ? `
      <div class="card chart-card" style="margin-bottom:var(--sp-4)">
        <h3 style="margin-bottom:10px">Study and Learning Activity</h3>
        ${lineChartSVG(activityVals, bucketLabels)}
      </div>` : ""}
